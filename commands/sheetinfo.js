...
module.exports = {
	...
	async execute(message, args, server) {
		...
		if (!sheet) {
			return message.channel.send(`Could not find the sheet \`${inputSheetName}\`!`);
		}

		try {
			message.channel.send(await splitEmbedsByFields(await getSheetInfo(sheet), 24, sheet.title, server));
		} catch (err) {
			console.log(err);
