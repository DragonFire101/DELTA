...
module.exports = {
	...
	aliases: [ 'config' ],
	description:
		"Changes the server's config values (Roster sheet name, id, rank system, and essential column headers).",
	args: true,
	...
	async execute(message, args, server) {
		...
		if (!actualHeader) {
			return message.channel.send(
				'Invalid header! `For the list of avaliable headers, take a look at the documentation.`'
			);
		}
		...
		try {
			...
			return message.channel.send(
				`Successfully set the \`${actualHeader}\` to \`${actualHeader == 'spreadsheetId'
					? 'REDACTED'
					: newHeader}\`!`
			);
		} catch (err) {
